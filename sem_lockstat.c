...
struct lock* allocate_node(int i,struct timespec curr_tm,struct mutex *resource)
{
	...
	//printk("\nCurrent : %s",current->comm);
	temp_lock_node->pid=current->pid;
	strcpy(temp_lock_node->type,"semaphore");
...
void end_module(void)
{
	...
	f = filp_open("ab2.txt", O_WRONLY|O_CREAT, 0);
	if(f == NULL)
    	...
    	else
	{
		while(head!=NULL)
		{
		printk("\n %u %s %u",head->pid,head->comm,head->res_addr);
